...
//                        std::cout << "Report matches: Bottom boundary reached \n";
  ...
  std::vector<std::tuple<int,int,int>> compute_set_maximal_match(vector<bool>& query) {
    ...
    // update divergence values
    CalculateDivergenceVal(virtual_insert_indices, query, zstart, bstart);
    auto outputMatches = [this] (unsigned site, unsigned hapId, unsigned matchStart, std::vector<std::tuple<int, int, int>>& matches) {
    ...
    for (unsigned k = 1; k <= this->N; ++k, prevDiv = newDiv) {
      ...
      if (zstart[k-1] < bstart[k-1]) {
        hapId = (hapId == UINT_MAX)? this->columns[k-1].pref_samples_end[this->columns[k-1].pref_samples_end.size() - 1] : this->phi.phi(hapId, k-1).value();
      }
...
#pragma omp parallel for default(none) \
  ...
  void PrintMemoryUsage(bool verbose=false){
    ...
    std::cout << "Memory of dcPBWT run_info(combined + ones + zeros) spsis : " << run_info_spsi_size_bytes << " bytes." << std::endl;
  ...
  void BuildFromVCF(std::string &filename) {
    ...
    if (std::ifstream inFile(filename); inFile.is_open()) {
      ...
      this->phi  = phi_ds(columns, this->M, this->N, sites_where_sample_beg, sites_where_sample_end, prefix_arr, div);
      assert(col == N);
      total_runs += freq.size();
