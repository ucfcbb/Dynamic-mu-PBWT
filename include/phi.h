...
class phi_ds {
    ...
    phi_ds(vector<dcpbwt_column> &columns, unsigned int M,
        ...
        for (unsigned int j = 0; j < total_haplotypes; j++) {
            ...
            if (j == this->phi_supp.size() - 1) {
                ...
                if (this->phi_inv_supp[last_pref[j]].size() == 0 ||
                    this->phi_inv_supp[last_pref[j]].at(this->phi_inv_supp[last_pref[j]].size() - 1)
                        != last_pref[j + 1]) {
    ...
    unsigned int plcp(unsigned int pref, unsigned int col) {
        if (col == 0 || pref == UINT_MAX || (phi(pref, col).value() == phi_inv(pref, col).value())
            || (phi(pref, col).value() == pref)) {
            return col;
        }
        auto tmp_col = this->phi_vec[pref].rank1(col);
        if (tmp_col == this->phi_supp[pref].size()) {
    ...
    // Serialize phi data structure
    size_t serialize(std::ostream &out) const {
        size_t bytes = 0;
        out.write((char *) &this->total_haplotypes, sizeof(this->total_haplotypes));
        bytes += sizeof(this->total_haplotypes);
        out.write((char *) &this->total_sites, sizeof(this->total_sites));
        bytes += sizeof(this->total_sites);

        size_t bits = 0;
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_vec[hap].serialize(out);
            bits += phi_vec[hap].bit_size();
        }

        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_inv_vec[hap].serialize(out);
            bits += phi_inv_vec[hap].bit_size();
        }

        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_supp[hap].serialize(out);
            bits += phi_supp[hap].bit_size();
        }

        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_inv_supp[hap].serialize(out);
            bits += phi_inv_supp[hap].bit_size();
        }
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_supp_lcp[hap].serialize(out);
            bits += phi_supp_lcp[hap].bit_size();
        }
        bytes += (bits/8);
        return bytes;
    }

    // Load or de-serialize phi data structure
    void load(std::istream &in) {
        in.read((char *) &this->total_haplotypes, sizeof(this->total_haplotypes));
        in.read((char *) &this->total_sites, sizeof(this->total_sites));

        this->phi_vec = std::vector<suc_bv>(this->total_haplotypes);
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_vec[hap].load(in);
        }

        this->phi_inv_vec = std::vector<suc_bv>(this->total_haplotypes);
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_inv_vec[hap].load(in);
        }

        this->phi_supp = std::vector<packed_spsi>(this->total_haplotypes);
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_supp[hap].load(in);
        }

        this->phi_inv_supp = std::vector<packed_spsi>(this->total_haplotypes);
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_inv_supp[hap].load(in);
        }

        this->phi_supp_lcp = std::vector<packed_spsi>(this->total_haplotypes);
        for (unsigned int hap = 0; hap < this->total_haplotypes; ++hap) {
            phi_supp_lcp[hap].load(in);
        }
    }
